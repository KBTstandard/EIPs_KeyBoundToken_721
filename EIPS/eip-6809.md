...
Parameter 4 `_allFunds`[^11]: This is a boolean value. When false, the `transfer` function takes into consideration Parameters 1, 2 and 3. If the value is true, the `transfer` function will revert to a **Default Behavior**[^4], the same as a traditional ERC-721.
...
[^15]: The `keyWallet1` is 1 of the 2 **Key Wallets** set when calling the `addBindings` function.
[^16]: The `keyWallet2` is 1 of the 2 **Key Wallets** set when calling the `addBindings` function.